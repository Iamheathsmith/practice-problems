...
// #4 number to string, then reverse it, then add it to old, then check if its a palindrome, then call it again to it finds one.

function reverseNumber(arr: number, count: number = 1) {
  console.log('inside', arr)
  let reversed:number = parseInt(arr.toString().split('').reverse().join(''))
  console.log('reversed', reversed)
  let newTotal:number = arr + reversed;
  console.log('newTotal', newTotal);

  let check:string[] = newTotal.toString().split('')
  let end:number = check.length - 1;
