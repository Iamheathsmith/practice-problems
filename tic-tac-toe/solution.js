...
module.exports = new class {
  ...
  checkDown(grid, start, count=grid.length) {
    if (count === 0) {
      return 'winner, matched down ' + grid[0][start];
    }
  ...
  checkDiagonalTD(grid, count=grid.length) {
    if (count === 0) {
      return 'winner, matched diagonal ' + grid[0][0];
    }
  ...
  checkDiagonalBU(grid, count=grid.length, top=0) {
    if (count === 0) {
      return 'winner, matched diagonal ' + grid[grid.length -1][0];
    }
  ...
  checkAcross(grid, count=grid.length) {
    if (count === 0) {
      return 'winner, matched across ' + grid[0];
    }
